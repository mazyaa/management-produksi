...
>
    <div x-data='produksiForm(@json(old() ?: []))'>
        <form method="POST" action="{{ route('produksis.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <x-form-select label="Shift" name="shift_id" required placeholder="Pilih Shift...">
                    @foreach($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id') == $shift->id ? 'selected' : '' }}>{{ $shift->nama_shift }}</option>
                    @endforeach
                </x-form-select>
                <x-form-select label="Mesin" name="mesin_id" required placeholder="Pilih Mesin...">
                    @foreach($mesins as $mesin)
                        <option value="{{ $mesin->id }}" {{ old('mesin_id') == $mesin->id ? 'selected' : '' }}>{{ $mesin->kode_mesin }}</option>
                    @endforeach
                </x-form-select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-form-select label="Part" name="part_id" required placeholder="Pilih Part...">
                    @foreach($parts as $part)
                        <option value="{{ $part->id }}" {{ old('part_id') == $part->id ? 'selected' : '' }}>{{ $part->nomor_part }} - {{ $part->nama_part }}</option>
                    @endforeach
                </x-form-select>
                <x-form-input
                    label="Catatan"
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" @click="$dispatch('close-modal', { name: 'create-produksi' })" class="btn-secondary">Batal</button>
